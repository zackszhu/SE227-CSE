...
sig_handler(int no) {
    ...
    case SIGINT:
        yfs->commit(version);
        printf("commit a new version %d\n", version);
        version++;
        break;
    case SIGUSR1:
        version--;
        printf("to previous version %d\n", version);
        yfs->gotoVersion(version);
        break;
    case SIGUSR2:
        version++;
        printf("to next version %d\n", version);
        yfs->gotoVersion(version);
        break;
    }
}
...
// origin version, not suitable for test-d
        ...
        mode_t mode, struct fuse_entry_param *e, int type)
{
    ...
    e->entry_timeout = 0.0;
    e->generation = 0;
    std::cout << "#Azard: In fuse->createhealper\n";
...
void 
fuseserver_utime(fuse_req_t req, fuse_ino_t ino)
{
    fuse_reply_err(req, ENOSYS);
}
